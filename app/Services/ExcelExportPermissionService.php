...
/**
 * Service: ExcelExportPermissionService
 * 
 * MỤC ĐÍCH:
 * Service kiểm tra quyền xuất Excel dựa trên subscription của organization
 * - Kiểm tra organization có subscription active và feature enable_data_export được bật không
 * 
 * LUỒNG XỬ LÝ CHÍNH:
 * 1. canExportExcel(): Kiểm tra organization có quyền xuất Excel không
 * 2. requireExportPermission(): Yêu cầu quyền, nếu không có thì throw exception hoặc trả về JSON error
 * 3. getExportStatus(): Lấy trạng thái quyền xuất Excel của organization
 * 4. getOrganization(): Lấy organization instance từ parameter hoặc từ user hiện tại
 * 
 * DỮ LIỆU ĐỌC TỪ:
 * - Model: Organization (bảng organizations) - Kiểm tra subscription và feature enable_data_export
 ...
 * 
 * LƯU Ý:
 * - Feature key: 'enable_data_export' - phải được enable trong subscription plan
 * - Organization phải có active subscription (status: 'trial' hoặc 'active')
 * - Nếu không có quyền, trả về 403 với message thân thiện
 */
class ExcelExportPermissionService
{
    const FEATURE_KEY = 'enable_data_export'; // Key của feature xuất Excel → Dùng để check trong subscription plan

    /**
     * Kiểm tra organization có quyền xuất Excel không
     * 
     * MỤC ĐÍCH:
     * Kiểm tra organization có subscription active và feature enable_data_export được bật không
     * 
     ...
     * 4. Kiểm tra feature enable_data_export có được bật không
     * 5. Trả về true/false
     * 
     * DỮ LIỆU ĐỌC TỪ:
     ...
     */
    public function canExportExcel($organization = null): bool
    {
        try {
            ...
            if (!$org) { // Nếu không tìm thấy organization
                Log::warning('ExcelExportPermissionService: Organization not found'); // Ghi log warning → Để debug
                return false; // Trả về false → Không có quyền
            }

            if (!$org->hasActiveSubscription()) { // Nếu organization không có subscription active
                Log::info('ExcelExportPermissionService: Organization does not have active subscription', [
                    'organization_id' => $org->id
                ]); // Ghi log info → Để tracking
                return false; // Trả về false → Không có quyền
            }

            $canExport = $org->canUseFeature(self::FEATURE_KEY); // Kiểm tra feature enable_data_export → Có được bật trong subscription plan không
            
            if (!$canExport) { // Nếu feature không được bật
                Log::info('ExcelExportPermissionService: Excel export feature is not enabled', [
                    'organization_id' => $org->id,
                    'feature_key' => self::FEATURE_KEY
                ]); // Ghi log info → Để tracking
            }

            return $canExport; // Trả về kết quả → true nếu có quyền, false nếu không

        } catch (\Exception $e) {
            Log::error('ExcelExportPermissionService: Error checking export permission', [
                'error' => $e->getMessage(),
    ...
    /**
     * Yêu cầu quyền xuất Excel, nếu không có thì throw exception hoặc trả về JSON error
     * 
     * MỤC ĐÍCH:
     * Kiểm tra quyền xuất Excel, nếu không có quyền thì dừng request và trả về lỗi 403 với message thân thiện
     * 
     * INPUT:
     * - organization: Organization instance, ID, hoặc null (sẽ lấy từ user hiện tại)
     ...
     * 
     * LUỒNG XỬ LÝ:
     * 1. Kiểm tra quyền bằng canExportExcel()
     ...
     */
    public function requireExportPermission($organization = null, ?string $message = null): void
    {
        if (!$this->canExportExcel($organization)) { // Nếu không có quyền
            $errorMessage = $message ?? 'Gói dịch vụ của bạn không hỗ trợ tính năng xuất Excel. Vui lòng nâng cấp gói để sử dụng tính năng này.'; // Message lỗi → Thân thiện với user
    ...
    /**
     * Lấy trạng thái quyền xuất Excel của organization
     * 
     * MỤC ĐÍCH:
     * Lấy thông tin chi tiết về quyền xuất Excel: có subscription không, feature có enabled không,
     ...
     * 
     * OUTPUT:
     * - array: Thông tin trạng thái {can_export, has_subscription, feature_enabled, organization_id, subscription_status}
     * 
     * LUỒNG XỬ LÝ:
     ...
     * - Bảng organizations: Kiểm tra subscription và feature
     */
    public function getExportStatus($organization = null): array
    {
        $org = $this->getOrganization($organization); // Lấy organization instance → Từ parameter hoặc user hiện tại
        ...
        return [
            'can_export' => $featureEnabled, // Có thể xuất Excel không → Kết quả cuối cùng
