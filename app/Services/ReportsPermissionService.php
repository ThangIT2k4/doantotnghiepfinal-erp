...
/**
 * Service: ReportsPermissionService
 * 
 * MỤC ĐÍCH:
 * Service kiểm tra quyền sử dụng tính năng Báo cáo nâng cao dựa trên subscription của organization
 * - Kiểm tra organization có subscription active và feature enable_reports được bật không
 * 
 * LUỒNG XỬ LÝ CHÍNH:
 * 1. canAccessReports(): Kiểm tra organization có quyền truy cập báo cáo nâng cao không
 * 2. requireReportsPermission(): Yêu cầu quyền, nếu không có thì throw exception hoặc trả về JSON error
 * 3. getReportsStatus(): Lấy trạng thái quyền báo cáo của organization
 * 4. getOrganization(): Lấy organization instance từ parameter hoặc từ user hiện tại
 * 
 * DỮ LIỆU ĐỌC TỪ:
 * - Model: Organization (bảng organizations) - Kiểm tra subscription và feature enable_reports
 ...
 * 
 * LƯU Ý:
 * - Feature key: 'enable_reports' - phải được enable trong subscription plan
...
{
    const FEATURE_KEY = 'enable_reports'; // Key của feature báo cáo nâng cao → Dùng để check trong subscription plan

    /**
     * Kiểm tra organization có quyền truy cập báo cáo nâng cao không
     * 
     * MỤC ĐÍCH:
     * Kiểm tra organization có subscription active và feature enable_reports được bật không
     * 
     ...
     * 4. Kiểm tra feature enable_reports có được bật không
     * 5. Trả về true/false
     * 
     * DỮ LIỆU ĐỌC TỪ:
     ...
     */
    public function canAccessReports($organization = null): bool
    {
        try {
            ...
            if (!$org) { // Nếu không tìm thấy organization
                Log::warning('ReportsPermissionService: Organization not found'); // Ghi log warning → Để debug
                return false; // Trả về false → Không có quyền
            }

            if (!$org->hasActiveSubscription()) { // Nếu organization không có subscription active
                Log::info('ReportsPermissionService: Organization does not have active subscription', [
                    'organization_id' => $org->id
                ]); // Ghi log info → Để tracking
                return false; // Trả về false → Không có quyền
            }

            $canAccess = $org->canUseFeature(self::FEATURE_KEY); // Kiểm tra feature enable_reports → Có được bật trong subscription plan không
            
            if (!$canAccess) { // Nếu feature không được bật
                Log::info('ReportsPermissionService: Advanced reports feature is not enabled', [
                    'organization_id' => $org->id,
                    'feature_key' => self::FEATURE_KEY
                ]); // Ghi log info → Để tracking
            }

            return $canAccess; // Trả về kết quả → true nếu có quyền, false nếu không

        } catch (\Exception $e) {
            Log::error('ReportsPermissionService: Error checking reports permission', [
                'error' => $e->getMessage(),
    ...
    /**
     * Yêu cầu quyền báo cáo nâng cao, nếu không có thì throw exception hoặc trả về JSON error
     * 
     * MỤC ĐÍCH:
     * Kiểm tra quyền báo cáo nâng cao, nếu không có quyền thì dừng request và trả về lỗi 403 với message thân thiện
     * 
     * INPUT:
     * - organization: Organization instance, ID, hoặc null (sẽ lấy từ user hiện tại)
     ...
     * 
     * LUỒNG XỬ LÝ:
     * 1. Kiểm tra quyền bằng canAccessReports()
     ...
     */
    public function requireReportsPermission($organization = null, ?string $message = null): void
    {
        if (!$this->canAccessReports($organization)) { // Nếu không có quyền
            $errorMessage = $message ?? 'Gói dịch vụ của bạn không hỗ trợ tính năng Báo cáo nâng cao. Vui lòng nâng cấp gói để sử dụng tính năng này.'; // Message lỗi → Thân thiện với user
    ...
    /**
     * Lấy trạng thái quyền báo cáo của organization
     * 
     * MỤC ĐÍCH:
     * Lấy thông tin chi tiết về quyền báo cáo: có subscription không, feature có enabled không,
     ...
     * 
     * OUTPUT:
     * - array: Thông tin trạng thái {can_access, has_subscription, feature_enabled, organization_id, subscription_status}
     * 
     * LUỒNG XỬ LÝ:
     ...
     * - Bảng organizations: Kiểm tra subscription và feature
     */
    public function getReportsStatus($organization = null): array
    {
        $org = $this->getOrganization($organization); // Lấy organization instance → Từ parameter hoặc user hiện tại
        ...
        return [
            'can_access' => $featureEnabled, // Có thể truy cập không → Kết quả cuối cùng
