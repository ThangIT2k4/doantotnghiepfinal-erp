...
/**
 * ============================================================================
 * FIX DEPLOY 500 ERROR - Simple Version (Không cần Laravel)
 * ============================================================================
 * 
 * Script này tự động xử lý các vấn đề thường gặp khi deploy gây lỗi 500
 * Phiên bản đơn giản - không cần load Laravel
 * 
 * CÁCH DÙNG:
 * 1. Upload file này lên root của project trên server
 * 2. Truy cập: https://your-domain.com/fix-deploy-500-simple.php
...
<head>
    ...
    <title>Fix Deploy 500 Error - Simple</title>
    <style>
...
<body>
    <div class="container">
        <h1>🔧 Fix Deploy 500 Error (Simple)</h1>
        ...
        foreach ($cacheFiles as $file) {
            if (file_exists($file)) {
                if (@unlink($file)) {
                    ...
                    $warnings[] = "Không thể xóa: $file (có thể do permissions)";
                    echo '<div class="result warning"><span class="icon">⚠️</span> Không thể xóa: <code>' . htmlspecialchars($file) . '</code> (Permission issue?)</div>';
                }
            }
        }
        ...
        if (file_exists('.env')) {
            ...
            if (preg_match('/APP_KEY=base64:[A-Za-z0-9+\/=]+/', $env)) {
                ...
                if ($updated && @file_put_contents('.env', $env)) {
                    $fixes[] = "Đã tạo và cập nhật APP_KEY";
                    echo '<div class="result success"><span class="icon">✅</span> Đã tạo và cập nhật APP_KEY trong .env</div>';
                } else {
                    $errors[] = "Không thể cập nhật APP_KEY trong .env (có thể do permissions)";
                    echo '<div class="result error"><span class="icon">❌</span> Không thể cập nhật APP_KEY - Cần set thủ công</div>';
                    echo '<div class="result info"><span class="icon">ℹ️</span> APP_KEY cần set trong .env: <code>' . htmlspecialchars($newKey) . '</code></div>';
                }
            }
        } else {
            $errors[] = "File .env không tồn tại";
            echo '<div class="result error"><span class="icon">❌</span> File .env không tồn tại - Cần tạo file .env từ .env.example</div>';
        }
        ...
        foreach ($dirs as $dir => $name) {
            if (is_dir($dir)) {
                if (is_writable($dir)) {
                    ...
                    $warnings[] = "$name không có quyền ghi";
                    echo '<div class="result warning"><span class="icon">⚠️</span> ' . $name . ' KHÔNG có quyền ghi - Cần set permission 755 hoặc 775 trong cPanel File Manager</div>';
                }
            } else {
        ...
        echo '<strong>Cảnh báo:</strong> ' . count($warnings) . ' vấn đề<br>';
        echo '<strong>Còn lỗi:</strong> ' . count($errors) . ' vấn đề';
        echo '</p>';
        
        if (count($fixes) > 0) {
            echo '<div style="text-align: left; margin-top: 15px; background: white; padding: 15px; border-radius: 6px;">';
        ...
        if (count($warnings) > 0) {
            echo '<div style="text-align: left; margin-top: 15px; background: #fff3cd; padding: 15px; border-radius: 6px;">';
            echo '<strong>⚠️ Cảnh báo:</strong><ul style="margin-left: 20px; margin-top: 10px;">';
        ...
        <div class="warning-box">
            ⚠️ BẢO MẬT: XÓA FILE fix-deploy-500-simple.php SAU KHI DÙNG XONG!
        </div>

        <div style="text-align: center; margin-top: 30px;">
            <a href="clear-cache-deploy.php" style="background: #667eea; color: white; padding: 12px 24px; border-radius: 6px; text-decoration: none; display: inline-block; margin: 5px;">🧹 Clear Cache</a>
            <a href="generate-key.php" style="background: #28a745; color: white; padding: 12px 24px; border-radius: 6px; text-decoration: none; display: inline-block; margin: 5px;">🔑 Generate APP_KEY</a>
        </div>
    </div>
</body>
</html>
