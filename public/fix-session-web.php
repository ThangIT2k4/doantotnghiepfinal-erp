 ...
 * 
 * Truy cập: https://ZoroRMS.click/fix-session-web.php?token=fix_session_2024
...
if (!isset($_GET['token']) || $_GET['token'] !== 'fix_session_2024') {
    ...
    <body>
        <div class="error">
            ...
            <p><strong>URL đúng:</strong> <code>?token=fix_session_2024</code></p>
        </div>
    </body>
    </html>
    ');
...
// Find vendor path - vendor is in project root, not in public
// On hosting, structure is usually: public_html/ (root) contains vendor/, public/ contains index.php
$vendorPath = null;
$bootstrapPath = null;

// Try different possible paths
$possiblePaths = [
    __DIR__.'/../vendor/autoload.php',  // Standard Laravel structure: public/../vendor/
    dirname(__DIR__).'/vendor/autoload.php',  // Alternative
    realpath(__DIR__.'/../vendor/autoload.php'),  // With realpath
];

foreach ($possiblePaths as $path) {
    if ($path && file_exists($path)) {
        $vendorPath = $path;
        // Set bootstrap path relative to vendor
        $bootstrapPath = dirname($path).'/../bootstrap/app.php';
        if (!file_exists($bootstrapPath)) {
            $bootstrapPath = dirname(dirname($path)).'/bootstrap/app.php';
        }
        break;
    }
}

// If still not found, try to detect from current working directory
if (!$vendorPath) {
    $currentDir = getcwd();
    if ($currentDir) {
        ...
        if (file_exists($testPath)) {
            $vendorPath = $testPath;
            $bootstrapPath = $currentDir.'/bootstrap/app.php';
        }
    }
}
...
if (!$vendorPath || !file_exists($vendorPath)) {
    ...
    $checkedPaths[] = ($currentDir ?? 'N/A') . '/vendor/autoload.php';
    
    die('
    <div style="padding: 20px; background: #fee; border: 1px solid #f00;">
        <h2>Error: vendor/autoload.php not found</h2>
        <p><strong>Current file location:</strong> ' . htmlspecialchars(__FILE__) . '</p>
        <p><strong>Current directory (__DIR__):</strong> ' . htmlspecialchars(__DIR__) . '</p>
        <p><strong>Parent directory:</strong> ' . htmlspecialchars(dirname(__DIR__)) . '</p>
        <p><strong>Working directory:</strong> ' . htmlspecialchars(getcwd() ?: 'N/A') . '</p>
        <p><strong>Paths checked:</strong></p>
        <ul>
            ' . implode('', array_map(function($path) {
                $exists = file_exists($path) ? ' ✓ EXISTS' : ' ✗ NOT FOUND';
                return '<li>' . htmlspecialchars($path) . $exists . '</li>';
            }, array_filter($checkedPaths))) . '
        </ul>
        <p><strong>Solution:</strong></p>
        <ol>
            <li>Ensure <code>vendor/</code> folder exists in project root (same level as <code>public/</code>)</li>
            <li>If missing, run <code>composer install</code> in project root via SSH or cPanel Terminal</li>
            <li>If you don\'t have terminal access, upload <code>vendor/</code> folder via FTP/cPanel</li>
        </ol>
    ...
    <div class="container">
            ...
            try {
                ...
                if ($hasTable) {
                    ...
                    try {
                        ...
                        echo "<div class='error'>✗ Lỗi khi chạy migration: " . htmlspecialchars($e->getMessage()) . "</div>";
            ...
            try {
                ...
                foreach ($commands as $cmd => $name) {
                    try {
                        ...
                        echo "<div class='warning'>⚠️ Error clearing $name: " . htmlspecialchars($e->getMessage()) . "</div>";
                    }
                }
            } catch (Exception $e) {
                echo "<div class='error'>Error: " . htmlspecialchars($e->getMessage()) . "</div>";
        ...
        <?php elseif ($action === 'test_session'): ?>
            ...
            try {
                ...
                echo "<div class='error'>✗ Session test ERROR: " . htmlspecialchars($e->getMessage()) . "</div>";
            }
            ?>
            <br>
            <a href="?token=fix_session_2024&action=check"><button>← Quay lại</button></a>
            ...
            try {
                ...
                if (!File::exists($envPath)) {
                    ...
                    try {
                        ...
                        echo "<div class='error'>✗ Không thể ghi file .env: " . htmlspecialchars($e->getMessage()) . "</div>";
        ...
        <div class="danger-zone">
            ...
            <ol>
                ...
                <li>Tìm file <code>public/fix-session-web.php</code></li>
                <li>Xóa file này</li>
            </ol>
        </div>
