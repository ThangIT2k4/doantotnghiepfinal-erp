// SePay Bank Data for QR Payment Integration
window.SePayBanks = {
    "no_banks": "52",
    "data": [
        {
...
// Helper functions for SePay bank integration
window.SePayBankHelper = {
    // Get supported banks only
    getSupportedBanks: function() {
        return window.SePayBanks.data.filter(bank => bank.supported === true);
    },
    
    // Get bank by code
    getBankByCode: function(code) {
        return window.SePayBanks.data.find(bank => bank.code === code);
    },
    
    // Get bank by BIN
    getBankByBin: function(bin) {
        return window.SePayBanks.data.find(bank => bank.bin === bin);
    },
    
    // Generate QR payment data for SePay
    generateQRData: function(bankCode, accountNumber, amount, description) {
        ...
        return {
            ...
            qr_data: `${bank.bin}|${accountNumber}|${amount}|${description}`
        };
    },
    
    // Generate SePay QR Code URL
    generateSePayQRUrl: function(accountNumber, bankName, amount, description) {
        const baseUrl = 'https://qr.sepay.vn/img';
        const params = new URLSearchParams();
        
        // Required parameters
        params.append('acc', accountNumber);
        ...
        if (description && description.trim()) {
            params.append('des', description);
        }
        
        return `${baseUrl}?${params.toString()}`;
    },
    
    // Get SePay bank name from bank code
    getSePayBankName: function(bankCode) {
        const bank = this.getBankByCode(bankCode);
        if (!bank) {
            throw new Error('Bank not found');
        }
        
        // SePay requires specific bank names, map from our data
        const sepayBankNames = {
            'ICB': 'VietinBank',
            'VCB': 'Vietcombank', 
            'MB': 'MBBank',
            'ACB': 'ACB',
        ...
        return sepayBankNames[bankCode] || bank.short_name;
    }
};
