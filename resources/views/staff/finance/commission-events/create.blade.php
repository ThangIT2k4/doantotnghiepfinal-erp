...
<main class="main-content">
    <div class="container-fluid">
        ...
        @include('staff.components.index-page-header', [
            'title' => 'Tạo Sự kiện Hoa hồng',
            'subtitle' => 'Tạo sự kiện hoa hồng mới cho nhân viên',
    ...
    <form action="{{ route('staff.commission-events.store') }}" method="POST">
        @csrf
        <div class="row">
            <!-- Basic Information -->
            <div class="col-lg-8">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="datetime-local" class="form-control @error('occurred_at') is-invalid @enderror" 
                                       name="occurred_at" id="occurred_at" value="{{ old('occurred_at', now()->format('Y-m-d\TH:i')) }}" required>
                                @error('occurred_at')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control money-input @error('amount_base') is-invalid @enderror" 
                                       name="amount_base" id="amount_base" value="{{ old('amount_base') ? number_format(old('amount_base'), 0, ',', '.') : '' }}" 
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control money-input @error('commission_total') is-invalid @enderror" 
                                       name="commission_total" id="commission_total" value="{{ old('commission_total') ? number_format(old('commission_total'), 0, ',', '.') : '' }}" 
...
function calculateCommission() {
    ...
    if (!policySelect || !amountInput || !calculatedInput || !totalInput || !preview) {
        console.error('One or more required elements not found for commission calculation');
        return;
    }
    ...
    if (!totalInput.value) {
        totalInput.value = calculated > 0 ? calculated.toLocaleString('vi-VN').replace(/,/g, '.') : '';
    }
...
function validateForm() {
    ...
    if (isNaN(amount) || amount <= 0) {
        Notify.warning('Số tiền gốc phải là số và lớn hơn 0.', 'Dữ liệu không hợp lệ!');
        amountElement.focus();
        return false;
    }

    // Validate date
    const dateElement = document.getElementById('occurred_at');
    if (!dateElement) {
        console.error('Element with id "occurred_at" not found');
        Notify.error('Không tìm thấy trường ngày xảy ra.', 'Lỗi hệ thống!');
        return false;
    }
    const occurredAt = new Date(dateElement.value);
    const now = new Date();
...
if (leaseSelect) {
    ...
    // Initialize unit_id if lease is pre-selected
